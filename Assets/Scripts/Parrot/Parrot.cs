...
public class Parrot : MonoBehaviour
{
	...
	[SerializeField] private AudioClip squawkClip;
	[Tooltip("Seconds between rapid squawks")]
	[SerializeField] private float squawkDelay = 0.5f;
	...
	private void OnSeePlayer()
	{
		...
		if (currentSuspicion >= maxSuspicion)
		{
			...
			{
				distractionLoop = StartCoroutine(DistractionSoundLoop());
			}
	...
	public IEnumerator DistractionSoundLoop()
	{
		...
		for(int i = 0; i < 3; i++)
		{
			...
			if (AudioManager.Instance != null && squawkClip != null)
			{
				AudioManager.Instance.PlaySFX(squawkClip);
			}
			//Instantiate(distractionPrefab, noiseMaker.transform.position, noiseMaker.transform.rotation);
			
			yield return new WaitForSeconds(squawkDelay);
		}
		CurrentSuspicion = 0f;
		// short cooldown to prevent instant re-triggering
		yield return new WaitForSeconds(1.0f);

		distractionLoop = null;
